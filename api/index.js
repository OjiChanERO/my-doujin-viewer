...
module.exports = async (req, res) => {
    ...
    try {
        ...
        switch (q) {
            case 'search':
                // Handles searching for a specific term.
                data = await api.search(term || '', parseInt(page) || 1);
                break;
            
            // *** THIS IS THE DEFINITIVE FIX ***
            // The 'popular' query now correctly performs a search for an empty string,
            // which the library uses to fetch the most recent/popular galleries.
            case 'popular':
